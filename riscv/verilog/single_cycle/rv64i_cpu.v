// rv64i_cpu.v — RISC-V 64IM + Zicsr + Zifencei 單週期處理器
...
module rv64i_cpu (
    ...
    always @(*) begin
        if (!csr_op) begin
            ...
            case (funct3)
                3'b001: csr_wdata = csr_rs1_val;                              // CSRRW
                3'b010: csr_wdata = csr_rdata | csr_rs1_val;                  // CSRRS
                3'b011: csr_wdata = csr_rdata & ~csr_rs1_val;                 // CSRRC
                3'b101: csr_wdata = csr_rs1_val;                              // CSRRWI
                3'b110: csr_wdata = csr_rdata | csr_rs1_val;                  // CSRRSI
                3'b111: csr_wdata = csr_rdata & ~csr_rs1_val;                 // CSRRCI
    ...
    always @(*) begin
        if (opcode == 7'b0110011) begin
            case ({funct7, funct3})
                ...
                {7'b0000001, 3'b010}: alu_ctrl = `ALU_MULH; // MULHSU uses same logic
                {7'b0000001, 3'b011}: alu_ctrl = `ALU_MULHU;
                {7'b0000001, 3'b100}: alu_ctrl = `ALU_DIV;
                {7'b0000001, 3'b101}: alu_ctrl = `ALU_DIVU;
                {7'b0000001, 3'b110}: alu_ctrl = `ALU_DIV;  // REM reuses DIV logic
                {7'b0000001, 3'b111}: alu_ctrl = `ALU_DIVU; // REMU reuses DIVU logic
        ...
        case (alu_ctrl)
            ...
            `ALU_DIV: begin
                if (alu_b == 0)
                    ...
                    alu_result = (1 << 63); // DIV overflow
                else if (funct3 == 3'b100) // DIV
                    alu_result = $signed(alu_a) / $signed(alu_b);
                else if (funct3 == 3'b110 && alu_b != 0) // REM
                    alu_result = $signed(alu_a) % $signed(alu_b);
                else
                    alu_result = alu_a;
            end
    ...
    // ====== Register Write-back ======
    wire [63:0] reg_wdata;
    assign reg_wdata = csr_op ? csr_rdata :
                       (reg_src == 2'b00) ? alu_result :
