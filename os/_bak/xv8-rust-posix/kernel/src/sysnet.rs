...
pub fn sys_socket(args: &SyscallArgs) -> Result<usize, Errno> {
    let Ok(port) = u16::try_from(args.get_int(0)) else {
        err!(Errno::EINVAL)
    };

    let socket_id =
        try_log!(SocketTable::open(Ipv4Addr::UNSPECIFIED, port, None).map_err(Errno::from));
...
pub fn sys_send(args: &SyscallArgs) -> Result<usize, Errno> {
...
pub fn sys_receive(args: &SyscallArgs) -> Result<usize, Errno> {
    let (_, file) = try_log!(args.get_file(0));
    let buf_addr = args.get_addr(1);
    let buf_len = args.get_int(2) as usize;
